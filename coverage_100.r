...
for (x in permute_range){
    path2 = paste("./run_",x,sep="") #path to downsampled files
    for (i in c(3:nrow(design),1:2)) {
      ...
      keep_names <-read.delim(paste("./DATA/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_exon",sep=""), header=F)
      ...
      keep_names <-read.delim(paste("./DATA/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_gwas",sep=""), header=F)
      keep <-which(snames$V1 %in% keep_names$V1) 
      ...
      # remove samples missing too much data
      write_dir <- paste(path2,"/SampleNames/keep",sep="")
      keep_names <-read.delim(paste("./DATA/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_neutral",sep=""), header=F)
      keep <-which(snames$V1 %in% keep_names$V1) 
