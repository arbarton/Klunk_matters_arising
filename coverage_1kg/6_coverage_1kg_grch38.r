...
min_n=5; min_maf=0.05
drop_samples_missing=.5

method='sliding'
path = "~/scratch_dir/Klunk_coverage_1000/smaller_set" ##insert path to DATA directory inluding the permutation files generated in the bash script under /SampleNames/permute/permute_{trial}
# for method=bins
if (method=='bins') {
...
neutral_table=list()
gwas_table=list()
permute_range <- c(1:100)

for(i in permute_range){
    path_run= paste(path,"/run_",i,sep="")
    info_gwas <- fread(paste(path_run,"/genoliks/immune.frq",sep=""))[,-c(3:4)]
    info_neut <- fread(paste(path_run,"/genoliks/neutral.frq",sep=""))[,-c(3:4)]
    info_exon <- fread(paste(path_run,"/genoliks/exon.frq",sep=""))[,-c(3:4)]
...
for (x in permute_range){
    path2 = paste(path,"/run_",x,sep="") #path to downsampled files
    for (i in c(3:nrow(design),1:2)) {
      ...
      name=paste(path2,"/genoliks/genolik.exons_",design$pop[i],"_", design$time[i],".genolik",sep="")
      n2=paste(design$pop[i],design$time[i],sep="_"); d <- as.data.frame(fread(name)); rm(name)
      snames=read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_exon",sep=""), header=F)
      ...
      write_dir <- paste(path2,"/SampleNames/keep",sep="")
      #keep <- which(get(paste("missing",n2, "exon", sep="_")) <= drop_samples_missing)
      keep_names <-read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_exon",sep=""), header=F)
      keep <-which(snames$V1 %in% keep_names$V1) 
      #print(paste(write_dir,n2,"exon",sep="_"))
      #assign(paste("keep",n2,"exon",sep="_"), value=snames[keep,]); write.table(snames[keep,], paste(write_dir,n2,"exon",sep="_"), row.names=F, col.names=F, sep="\t", quote=F)
      ...
      name=paste(path2,"/genoliks/genolik.gwas_",design$pop[i],"_", design$time[i],".genolik",sep="")
      n2=paste(design$pop[i],design$time[i],sep="_"); d <- as.data.frame(fread(name)); rm(name)
      snames=read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_gwas",sep=""), header=F)
      ncol(d)-2 == 3*nrow(snames)
      ...
      keep_names <-read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_gwas",sep=""), header=F)
      keep <-which(snames$V1 %in% keep_names$V1) 
      #assign(paste("keep",n2,"gwas",sep="_"), value=snames[keep,]); write.table(snames[keep,], paste(write_dir,n2,"gwas",sep="_"), row.names=F, col.names=F, sep="\t", quote=F)
      k2 <- as.data.frame(matrix(nrow=nrow(d), ncol=1)); for (k in keep) {k2 <- cbind(k2, d[,(k*3-2):(k*3)])}
      d <- k2[,-1]; rm(keep, k2, k); n <- ncol(d)/3
      # get the allele frequency, and the number of individuals it was called from
      info_gwas <- gwas_table[[x]]
      info_gwas[[paste(n2,"alternate",sep=".")]] <- rowMeans(d[,seq(3,n*3,3)],na.rm=T) + rowMeans(d[,seq(2,n*3,3)],na.rm=T)/2
      info_gwas[[paste(n2,"called",sep=".")]] <- (n*3-rowSums(is.na(d)))/3
      # Report number of samples included 
      ...
      name=paste(path2,"/genoliks/genolik.neutral_",design$pop[i],"_", design$time[i],".genolik",sep="")
      n2=paste(design$pop[i],design$time[i],sep="_"); d <- as.data.frame(fread(name)); rm(name)
      snames=read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_neutral",sep=""), header=F)
      ncol(d)-2 == 3*nrow(snames)
      ...
      # remove samples missing too much data
      write_dir <- paste(path2,"/SampleNames/keep",sep="")
      keep_names <-read.delim(paste("~/reich_home/genoliks/SampleNames/keep_", design$pop[i], "_", design$time[i],"_neutral",sep=""), header=F)
      keep <-which(snames$V1 %in% keep_names$V1) 
      #keep <- which(get(paste("missing",n2, "neutral", sep="_")) <= drop_samples_missing)
      assign(paste("keep",n2,"neutral",sep="_"), value=snames[keep,]); write.table(snames[keep,], paste(write_dir,n2,"neutral",sep="_"), row.names=F, col.names=F, sep="\t", quote=F)
      k2 <- as.data.frame(matrix(nrow=nrow(d), ncol=1)); for (k in keep) {k2 <- cbind(k2, d[,(k*3-2):(k*3)])}
      d <- k2[,-1]; rm(keep, k2, k); n <- ncol(d)/3
      # get the allele frequency, and the number of individuals it was called from
      info_neut <- neutral_table[[x]]
      info_neut[[paste(n2,"alternate",sep=".")]] <- rowMeans(d[,seq(3,n*3,3)],na.rm=T) + rowMeans(d[,seq(2,n*3,3)],na.rm=T)/2
      info_neut[[paste(n2,"called",sep=".")]] <- (n*3-rowSums(is.na(d)))/3
      # Report number of samples included 
...
for (x in permute_range){
    ...
    info_gwas <- info_gwas[,c(1:5, 6, 8, 10, 12, 14, 18, 19, 27, 30, 33, 36, 37:40)]
    info_neut <- info_neut[,c(1:5, 6, 8, 10, 12, 14, 18, 19, 27, 30, 33, 36, 37:40)]
...
#load("~/reich_home/DATA_part1.RData")
## Print some metadata
...
for (x in permute_range){
    ...
    info <- cbind(info, pvals[,6])
    colnames(info)[22] <- c("L13.pval")
    info_neut <- cbind(info_neut, neut_pvals[,6])
    colnames(info_neut)[22] <- c("L13.pval")
...
write.csv(London_p_maf10_e05,"London_p_maf10_e05_new.txt",row.names=FALSE)
write.csv(London_p_maf10_e01,"London_p_maf10_e01_new.txt",row.names=FALSE)
write.csv(London_p_maf30_e01,"London_p_maf30_e01_new.txt",row.names=FALSE)
